        ...
        public static Item[] FillKnapsack(Item[] items, int capacity)
        {
            ...
            for (int i = 1; i < items.Length; i++)
            {
                for (int c = 0; c <= capacity; c++)
                {
                    ...
                    int remainingSpace = c - items[i].Weight;
                    if (remainingSpace >= 0 && items[i].Price + maxPrice[i - 1, remainingSpace] > maxPrice[i, c])
                    {
                        maxPrice[i, c] = items[i].Price + maxPrice[i - 1, remainingSpace];
